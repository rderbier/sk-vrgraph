...
namespace StereoKitApp
{
    ...
    public class App
    {
        ...
        private static List<IDrawable> UITypeComponentList;
        private static VshapeLayout layoutForNodes;
        private static SphericalLayout layoutForSchema;
        ...
        private void initUI()
        {
            ...
            Vec3 point = new Vec3(-.8f, -0.8f, -0.20f);
            infoPose = new Pose(point, Quat.LookAt(point, initialPose.position, Vec3.Up));
            layoutForNodes = new VshapeLayout(objPose, 100, SKMath.Pi / 4.0f, 25 * U.cm);
            layoutForSchema = new SphericalLayout(initialPose, 0.6f, 25 * U.cm, 8 * U.cm);
        }
        ...
        public async void Init()
        {
            ...
            // create UIelement list from TypeElement list.
            UITypeComponentList = GraphTypeUtils.CreateGraphTypeUIElementList(new List<TypeElement>(graphSchema.Values));
            layoutForSchema.SetElementList(UITypeComponentList, 0, 0, 0);
        ...
        public void Step()
        {
            ...
            LogWindow();
            LayoutStatus schemaUIstatus = layoutForSchema.Draw();
            string selected = null;
            //var selected = GraphDisplayer.selectTypeInSchema();

            if (schemaUIstatus.isSelected)
            {
                selectedType = ((TypeElement)schemaUIstatus.selected.GetValue()).name;
            ...
            if ((selectedType != null) & (graphSchema != null))
            {
                GraphDisplayer.displayTypeInfo(graphSchema[selectedType], infoPose);
            }
